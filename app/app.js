...
// Render history cards from data.
...
// Try loading history from the D1-backed API; fall back to localStorage.
const loadHistory = async () => {
  ...
  try {
    ...
    console.warn("History API failed, using localStorage.", error);
    useHistoryApi = false;
    renderHistory(readHistory());
  }
};
  ...
  try {
    ...
    if (useHistoryApi) {
      try {
        ...
        console.warn("History API save failed, using localStorage.", error);
        useHistoryApi = false;
      }
    }
...
clearHistoryBtn.addEventListener("click", () => {
  if (useHistoryApi) {
    fetch("/api/history", { method: "DELETE" })
      .then(() => loadHistory())
      .catch((error) => {
        console.warn("History API clear failed, using localStorage.", error);
        useHistoryApi = false;
